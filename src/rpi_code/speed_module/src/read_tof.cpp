...
class ReadTofNode : public rclcpp::Node {
public:
    /* 생성자 (객체를 만들 때 자동 호출 + 초기설정) */
    // node: read_tof_node
	ReadTofNode() : Node("read_tof_node") {
        /* publisher */
		...
		sub_ = this->create_subscription<can_msgs::msg::Frame>(
				"/from_can_bus", 10,
				std::bind(&ReadTofNode::can_callback, this, std::placeholders::_1)
				);
		
		RCLCPP_INFO(this->get_logger(), "Read TOF Node (ID: 522)가 준비되었습니다."); // ROS2의 printf
	}
private:
    /* callback function */
	void can_callback(const can_msgs::msg::Frame::SharedPtr msg) {
		...
		if (msg->id == 0x200) {
			...
			// ****************************************************디버깅용 로그
			RCLCPP_INFO(this->get_logger(), "ToF 수신 (ID 522): %u mm", tofVal);
			// ****************************************************디버깅용 로그
...
int main(int argc, char* argv[]) {
	...
	rclcpp::spin(std::make_shared<ReadTofNode>()); // ReadTofNode 클래스로 생성하고 실행
	rclcpp::shutdown();
	return 0;
}